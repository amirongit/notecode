...
# DRF's version of reverse function can return an absolute URL using the
# request object, while django's version returns relative URL. the absolute
# URL might be needed when the API has different versions.
# The same difference exists between django's and DRF's versions of
# reverse_lazy.
...
class SnippetHightlight(generics.GenericAPIView):
    queryset = Snippet.objects.all()
    renderere_classes = [renderers.StaticHTMLRenderer]
